...
### Save to file or writer
    ...
    err := curl.File("www.baidu.com", "/tmp/index.html")
  	...
  	var w io.Writer
  	err, response := curl.Write("http://www.baidu.com", w)
...
### Just dial
    ...
    err, r, length, response := curl.Dial("http://weibo.com", "timeout=11")
...
func Dial(url string, opts ...interface{}) (
	err error, r io.ReadCloser, length int64, resp *http.Response,
) {
...
func Write(url string, w io.Writer, opts ...interface{}) (err error, resp *http.Response) {
	var r io.ReadCloser
	var length int64
	err, r, length, resp = Dial(url, opts...)
	if err != nil {
		return
	}
